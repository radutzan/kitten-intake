...
class UrlStateManager {
    ...
    _doUpdateUrl() {
        ...
        if (encoded) {
            ...
            url.search = `?${this.paramKey}=${encoded}&${this.dateParamKey}=${AppState.getIntakeDateISO()}`;
    ...
    decodeFromUrl(urlString = window.location.href) {
        ...
        // Session intake date travels as a separate param (absent in older links)
        const intakeDate = url.searchParams.get(this.dateParamKey);

        return {
            appState: {
                kittenCounter: kittenIndex - 1,
                intakeDate: /^\d{4}-\d{2}-\d{2}$/.test(intakeDate || '') ? intakeDate : null,
                activeKittens
            },
            kittens
        };
    ...
    clearUrlState() {
        ...
        url.searchParams.delete(this.dateParamKey);
        window.history.replaceState({}, '', url.toString());
    }
    ...
    urlMatchesLocalStorage() {
        try {
            ...
            // Compare intake dates (only when the link carries one)
            if (urlData.appState.intakeDate && urlData.appState.intakeDate !== localData.appState.intakeDate) {
                return false;
            }
